...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'users_suspended_by_id_fkey'
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'tutor_applications_reviewed_by_id_fkey'
      AND conrelid = '"tutor_applications"'::regclass
  ) THEN
    ALTER TABLE "tutor_applications"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'admin_audit_log_admin_id_fkey'
...
CREATE INDEX IF NOT EXISTS "admin_audit_log_target_type_target_id_idx"
